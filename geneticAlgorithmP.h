...
float fitnessRedProximity(vector<vector<Pixel>> &pCleanImage,Pixel pixel){
    ...
    for(int search = 1;search<=rangeSearched;search++){
        ...
        if("Red" == returnColor(pCleanImage[x-search][y])){
            ...
            if( (pCleanImage[x-search][y].getFitness()) <= 4){
                pixelFitness -=  0.2;
            }
        }
        ...
        if("Red" == returnColor(pCleanImage[x][y+search])){
            if( (pCleanImage[x][y+search].getFitness()) <= 4){
                pixelFitness -=  0.2;
            }
...
vector<Pixel> crossPixels(vector<Pixel> &bestPixels, Mat &imageFirstPopulation ){
    ...
    vector<int> randomIndexes = generateRandoms(199,200);
...
void mainGenetic(){
    ...
    Mat image2;
    //Mat finalImage = imageFirstPopulation;
    ...
    while(avgFitness<=0.5){ 
        Mat newImage = draftImage;
        newPixelPopulation = crossPixels(initialPixels, newImage);
        initialPixels  = newPixelPopulation;
        calculateFitness(newPixelPopulation,cleanImage);         //Calculates the fitness of the initial poblation   
        avgFitness = averageFitness(newPixelPopulation);
        generation++;
        cout<<"Fitness: "<<avgFitness<<endl;
    }
    /*imshow("Prueba",imageFirstPopulation);
    waitKey(0);*/
    ...
    resize(finalImage,image2,Size(700,700));
    imshow("Final",image2);
